...
interface TimesheetPdfPreviewV3Props {
  employee: Employee;
  month: number;
  year: number;
...
const TimesheetPdfPreviewV3 = ({ employee, month, year, dailyRecords, logoSrc }: TimesheetPdfPreviewV3Props) => {
  ...
  const handleDownloadPdfDirect = async () => {
    ...
    try {
      const doc = <TimesheetPdfDocumentV3
        employee={employee}
        month={month}
        year={year}
      ...
      const filename = `Folha_de_Ponto_V3_${employee.name.replace(/\s/g, '_')}_${month}_${year}.pdf`;
...
export default TimesheetPdfPreviewV3;