...
const styles = StyleSheet.create({
  ...
  cellBase: {
    borderRightWidth: 1.5, // Ajustado para 1.5pt
    borderBottomWidth: 1.5, // Ajustado para 1.5pt
    borderColor: '#000000',
    borderStyle: 'solid',
    padding: 2,
    textAlign: 'left',
          ...
          {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((day, index) => {
            ...
            return (
              <View style={styles.tableRow} key={day}>
                ...
                {/* Hora Extra Saída */}
                <Text style={[styles.colExtraTime, { borderRightWidth: 0 }, isLastRow && { borderBottomWidth: 0 }]}>{formatFullTime(record?.exit_time_2)}</Text>
              </View>
            );
          })}
