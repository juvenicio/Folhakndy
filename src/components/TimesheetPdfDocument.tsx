...
const styles = StyleSheet.create({
  ...
  // Estilos para as células internas (sem bordas no estilo base, serão aplicadas no JSX)
  cellBase: {
    borderColor: '#000000',
    borderStyle: 'solid',
    padding: 2,
  ...
  // Estilos para as células do cabeçalho da folha de ponto (sem bordas no estilo base)
  tableHeaderCell: {
    borderColor: '#000000',
    borderStyle: 'solid',
    padding: 1,
    ...
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.mainTableContainer}>
        ...
        <View style={styles.tableRow}>
          <View style={[styles.cellBase, { width: '100%', borderLeftWidth: 0, borderTopWidth: 0, borderRightWidth: 0, borderBottomWidth: 1.5 }]}>
            <Text style={{ flexGrow: 0 }}>Unidade de Trabalho: {employee.school_name || 'N/A'}</Text>
          </View>
        </View>
        {/* Row 2: Servidor(a) / Matrícula */}
        <View style={styles.tableRow}>
          <View style={[styles.cellBase, { width: '66.66%', fontSize: 10, borderLeftWidth: 0, borderTopWidth: 1.5, borderRightWidth: 1.5, borderBottomWidth: 1.5 }]}>
            <Text style={{ flexGrow: 0 }}>Servidor(a): {employee.name}</Text>
          </View>
          <View style={[styles.cellBase, { width: '33.33%', borderTopWidth: 1.5, borderRightWidth: 0, borderBottomWidth: 1.5 }]}>
            <Text style={{ flexGrow: 0 }}>Matrícula: {employee.registration_number}</Text>
          </View>
        </View>
        {/* Row 3: Cargo / Função / Turno */}
        <View style={styles.tableRow}>
          <View style={[styles.cellBase, { width: '33.33%', borderLeftWidth: 0, borderTopWidth: 1.5, borderRightWidth: 1.5, borderBottomWidth: 1.5 }]}>
            <Text style={{ flexGrow: 0 }}>Cargo: {employee.employee_type}</Text>
          </View>
          <View style={[styles.cellBase, { width: '33.33%', borderTopWidth: 1.5, borderRightWidth: 1.5, borderBottomWidth: 1.5 }]}>
        ...
        {/* Row 4: Vínculo / Mês / Ano (Last row of Employee Details) */}
        <View style={styles.tableRow}>
          <View style={[styles.cellBase, { width: '33.33%', borderLeftWidth: 0, borderTopWidth: 1.5, borderRightWidth: 1.5, borderBottomWidth: 1.5 }]}>
        ...
        <View style={styles.tableRow} fixed>
          ...
          <View style={[styles.tableHeaderCell, { width: '25%', flexDirection: 'column', padding: 0, borderRightWidth: 0, borderTopWidth: 1.5, borderBottomWidth: 1.5 }]}>
            ...
            <View style={{ flexDirection: 'row', width: '100%', flexGrow: 1 }}>
              <View style={[styles.tableHeaderCell, { width: '50%', borderLeftWidth: 1.5, borderTopWidth: 0, borderRightWidth: 1.5, borderBottomWidth: 0 }]}>
                <Text style={{ fontSize: 10, fontFamily: 'Calibri' }}>Entrada</Text>
                <Text style={{ fontFamily: 'Times-Roman', fontSize: 4 }}>(Horas | Minutos | Segundos)</Text>
              </View>
              <View style={[styles.tableHeaderCell, { width: '50%', borderLeftWidth: 1.5, borderTopWidth: 0, borderRightWidth: 0, borderBottomWidth: 0 }]}>
        ...
        {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((day, index) => {
          ...
          return (
            <View style={styles.tableRow} key={day}>
              <Text style={[styles.cellBase, styles.colDia, { borderLeftWidth: 0, borderTopWidth: 1.5, borderRightWidth: 1.5, borderBottomWidth: isLastDailyRecordRow ? 0 : 1.5 }]}>{day}</Text>
              <Text style={[styles.cellBase, styles.colTime, { borderTopWidth: 1.5, borderRightWidth: 1.5, borderBottomWidth: isLastDailyRecordRow ? 0 : 1.5 }]}>{displayTimeValue(record?.entry_time_1)}</Text>
              <Text style={[styles.cellBase, styles.colSignature, { borderTopWidth: 1.5, borderRightWidth: 1.5, borderBottomWidth: isLastDailyRecordRow ? 0 : 1.5 }]}>{displaySignatureValue}</Text>
              <Text style={[styles.cellBase, styles.colTime, { borderTopWidth: 1.5, borderRightWidth: 1.5, borderBottomWidth: isLastDailyRecordRow ? 0 : 1.5 }]}>{displayTimeValue(record?.exit_time_1)}</Text>
              <Text style={[styles.cellBase, styles.colSignature, { borderTopWidth: 1.5, borderRightWidth: 1.5, borderBottomWidth: isLastDailyRecordRow ? 0 : 1.5 }]}>{displaySignatureValue}</Text>
              {/* Hora Extra Entrada */}
              <Text style={[styles.cellBase, styles.colExtraTime, { borderTopWidth: 1.5, borderRightWidth: 1.5, borderBottomWidth: isLastDailyRecordRow ? 0 : 1.5 }]}>{formatFullTime(record?.entry_time_2)}</Text>
              {/* Hora Extra Saída */}
              <Text style={[styles.cellBase, styles.colExtraTime, { borderTopWidth: 1.5, borderRightWidth: 0, borderBottomWidth: isLastDailyRecordRow ? 0 : 1.5 }]}>{formatFullTime(record?.exit_time_2)}</Text>
            </View>
          );
        })}
        ...
        {/* Seção de Observação */}
        <View style={styles.tableRow}>
          <View style={[styles.cellBase, { width: '100%', padding: 3, borderLeftWidth: 0, borderTopWidth: 1.5, borderRightWidth: 0, borderBottomWidth: 1.5 }]}>
            <Text style={[styles.sectionTitle, { flexGrow: 0 }]}>Observação:</Text>
            <Text style={{ minHeight: 15, flexGrow: 0 }}></Text>
          </View>
        </View>

        {/* Seção de Justificativa/Horas Extras (Last row of the entire table) */}
        <View style={styles.tableRow}>
          <View style={[styles.cellBase, { width: '100%', padding: 3, borderLeftWidth: 0, borderTopWidth: 1.5, borderRightWidth: 0, borderBottomWidth: 0 }]}>
